...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const effectiveMaxFiles = layerCode && (layerCode.includes('.set') || layerCode === 'T' || layerCode === 'P') ? 5 : 1;
  ...
  const handleUploadComplete = useCallback((fileId: string, fileData: FileUploadResponse) => {
  ...
  const handleUploadError = useCallback((fileId: string, errorMessage: string) => {
  ...
  const handleFileUploaderError = useCallback((file: File, error: string) => {
    ...
    <Paper sx={{ p: 3, mb: 4 }}>
      ...
      <Typography variant="body2" color="text.secondary" paragraph>
        {layerCode && layerCode !== 'P' && layerCode !== 'T' && !layerCode.includes('.set') ? 
          ...
          `Upload one or more files for your asset. ${effectiveMaxFiles > 1 ? `You can upload up to ${effectiveMaxFiles} files for set type assets.` : ''}`
        }
      </Typography>
      <Divider sx={{ mb: 3 }} />
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <FileUploader
            ...
            showPreviews={false}
            uploadLabel={`Drag and drop files for ${layerCode ? `${layerCode} layer` : 'your asset'}, or click to select`}
          />
        </Grid>
