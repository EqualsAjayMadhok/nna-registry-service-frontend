...
import { Asset, VersionInfo, VersionChanges, FileChange, FieldChange, MetadataChange, AssetFile } from '../../types/asset.types';
...
const VersionHistory: React.FC<VersionHistoryProps> = ({ 
  ...
  const handleOpenContextMenu = (event: React.MouseEvent<HTMLButtonElement>, version: VersionInfo) => {
        ...
        <Paper variant="outlined" sx={{ p: 2 }}>
          {changes.metadataChanges.map((metaChange, index) => (
            <Box key={index} sx={{ mb: index < changes.metadataChanges!.length - 1 ? 2 : 0 }}>
              ...
              <DiffView
                oldValue={metaChange.oldValue}
                newValue={metaChange.newValue}
              />
        ...
        {newerVersion.changes ? (
          <>
            ...
            {(!newerVersion.changes.fields || newerVersion.changes.fields.length === 0) &&
             (!newerVersion.changes.metadataChanges || newerVersion.changes.metadataChanges.length === 0) &&
             (!newerVersion.changes.filesAdded || newerVersion.changes.filesAdded.length === 0) &&
             (!newerVersion.changes.filesModified || newerVersion.changes.filesModified.length === 0) &&
             (!newerVersion.changes.filesRemoved || newerVersion.changes.filesRemoved.length === 0) && (
              <Alert severity="info">
                No detailed changes detected between these versions.
              </Alert>
            )}
          </>
        ) : (
  ...
  return (
    <Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box>
            <Typography variant="subtitle1">
              Current Version: {asset.version.number}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Created by {asset.version.createdBy} on {format(new Date(asset.version.createdAt), 'PPP')}
            </Typography>
                  ...
                  {!compareMode && (
                    <>
                      <Tooltip title="More Actions">
                        <IconButton
                          edge="end"
                          onClick={(e) => handleOpenContextMenu(e, version)}
                        >
                          <MoreIcon />
                        </IconButton>
                      </Tooltip>
                  ...
                  {version.changes ? (
                    <>
                      ...
                      {(!version.changes.fields || version.changes.fields.length === 0) &&
                       (!version.changes.metadataChanges || version.changes.metadataChanges.length === 0) &&
                       (!version.changes.filesAdded || version.changes.filesAdded.length === 0) &&
                       (!version.changes.filesModified || version.changes.filesModified.length === 0) &&
                       (!version.changes.filesRemoved || version.changes.filesRemoved.length === 0) && (
                        <Alert severity="info">
      ...
      >
        <DialogTitle>
          Revert to Version {revertVersion?.number}
        </DialogTitle>
        <DialogContent>
          <DialogContentText paragraph>
            Are you sure you want to revert to version {revertVersion?.number}? 
