...
const AssetDetail: React.FC = () => {
  ...
  const refreshAsset = async () => {
    ...
    try {
      ...
      await loadRightsInformation(refreshedAsset.id);
  ...
  // Get selected file
  const selectedFile = asset.files && asset.files.length > 0 
    ? asset.files[selectedFileIndex] 
    : null;
    ...
    <Container maxWidth="lg">
      ...
      <Box sx={{ my: 3 }}>
        ...
        <Box display="flex" alignItems="center" flexWrap="wrap">
          ...
          {asset.category && (
            <Chip
              label={getFullTaxonomyPath(asset)}
              variant="outlined"
              sx={{ mr: 1, mb: 1 }}
            />
          )}
          ...
          {currentVersionNumber && asset.version && currentVersionNumber !== asset.version.number && (
            <Chip
          ...
          <Card>
            ...
            >
              ...
              <Tab 
                ...
                icon={currentVersionNumber !== asset.version.number ? <Badge color="secondary" variant="dot"><HistoryIcon /></Badge> : undefined}
                iconPosition="end"
              />
              ...
              <Tab 
                ...
                id="asset-tab-4" 
                aria-controls="asset-tabpanel-4"
              />
              
              <Tab 
                label="Rights Management" 
                id="asset-tab-5" 
                aria-controls="asset-tabpanel-5"
              />
            </Tabs>
            ...
            <TabPanel value={tabValue} index={1}>
              <CardContent>
                ...
                {!asset.files || asset.files.length === 0 ? (
                  <Alert severity="info">
                    This asset does not have any associated files.
                  </Alert>
                ) : (
                  <>
                    {/* File list */}
            ...
            <TabPanel value={tabValue} index={2}>
              <CardContent>
                ...
                {!asset.metadata || Object.keys(asset.metadata).length === 0 ? (
                  <Alert severity="info">
                    This asset does not have any additional metadata.
                  </Alert>
                ) : (
            ...
            <TabPanel value={tabValue} index={3}>
              <CardContent>
                ...
                {currentVersionNumber !== asset.version.number && (
                  ...
                  >
                    You are viewing version {currentVersionNumber} (current version is {asset.version.number})
                  </Alert>
                )}
            ...
            {canEdit && (
              <CardActions sx={{ borderTop: `1px solid ${theme.palette.divider}`, justifyContent: 'flex-end', px: 2, py: 1.5 }}>
                <Button
                  startIcon={<EditIcon />}
          ...
          <Card>
            <CardContent>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {assetRights && (
                  <Tooltip 
                    title={`Rights Status: ${assetRights.status.charAt(0).toUpperCase() + assetRights.status.slice(1)}`}
                    arrow
                  >
                    <Chip
                      label={assetRights.status.charAt(0).toUpperCase() + assetRights.status.slice(1)} 
                      color={
                        assetRights.status === 'verified' ? 'success' :
                        assetRights.status === 'partial' ? 'info' :
                        assetRights.status === 'pending' ? 'warning' :
                        assetRights.status === 'rejected' || assetRights.status === 'expired' ? 'error' :
                        'default'
                      }
                      size="small"
                      ...
                      <Grid item xs={12}>
                        {assetRights.clearances && assetRights.clearances.length > 0 ? (
                          <Typography variant="body2" sx={{ mt: 1 }}>
                            {assetRights.clearances.length} clearance record{assetRights.clearances.length !== 1 ? 's' : ''}
                          </Typography>
                        ) : (
